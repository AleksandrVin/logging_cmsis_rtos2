...
> 💡 **Профили в Docker Compose** позволяют иметь разные конфигурации в одном файле. Профиль `test` для тестов, `gdb` для отладки.
...
4. **Просмотр переменных**:
    ...
    ![GDB print variables](./gdb_vars.png)
...
2. **Потеря контекста** при прерываниях:
   ...
   Решение: установите breakpoint в обработчике прерывания
